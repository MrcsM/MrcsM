# Hello!
Hi! My name is Marcus Moquin, I am a 23-year-old developer. 

I obtained my Bachelor's Degree in Computer Science from Carleton University and graduated in Winter 2025.

I am currently working with [Java](https://www.java.com/en/) and [Python](https://www.python.org/) mostly while I have used [Swift](https://developer.apple.com/swift/), [Haskell](https://www.haskell.org/), [Kotlin](https://kotlinlang.org/), [JavaScript](https://www.javascript.com/), [C](https://en.wikipedia.org/wiki/C_(programming_language)) and [C++](https://isocpp.org/) in my university semesters. Before university, my first experience programming was in [Skript](https://github.com/SkriptLang/Skript), a basic coding language for the game, Minecraft.
#

On my page, you'll find most of my class assignments, tutorials, and some of my projects.
 - Assisting Java development on [ArcticMC](https://www.twitter.com/ArcticUHC) ~ May 2023 - Present
 - Quality Assurance/Minecraft Plugin Development at [Hoplite](https://www.hoplite.gg) ~ March 2025 - Present
 - AED Simulator Group Project: [found here](https://github.com/MrcsM/COMP3004FinalProject) ~ Fall 2023 (Nov. - Jan. 2024)

#
[![GitHub stats](https://github-readme-stats-five-sand.vercel.app/api?username=mrcsm&count_private=true&show_icons=true&theme=github_dark)](https://github.com/anuraghazra/github-readme-stats)

